...
| パターン | `patterns.json` に登録した正規表現パターンを選択する |
...
**パターン生成ウィザード**（「パターン生成 →」ボタン）  
現在のペアリストから正規表現パターンを自動生成・検証・保存する画面に切り替わる。

- テンプレート選択または手動入力で正規表現を編集
- 「検証実行」でキー抽出プレビューと整合性チェック
- 「保存」で `patterns.json` に追加（次回以降「パターン」方式で再利用可能）
