...
**比較実行**ボタンを押すと一括比較を開始し、完了後に **`★index.html`** および **`★index.xlsx`**（比較結果一覧）を生成する。
...
#### インデックスファイル（★index.html / ★index.xlsx）の内容
...
`★index.xlsx` は Excel 形式の同等ファイルで、リンク列をクリックすると個別差分 HTML を開ける。
