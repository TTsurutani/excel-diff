...
フォルダ内の複数Excelファイルを一括比較し、差分HTML群と`index.html`（一覧）を生成する。

**ペアリング方式**

| 方式 | 説明 |
|---|---|
| exact（同名ファイル） | 旧・新フォルダで同名のファイルを比較 |
| ペアJSONファイル | タブ④で探索・保存したペアJSONを直接指定 |
| パターン | タブ④で登録した正規表現パターンIDを選択 |

その他オプション（出力フォルダ・比較シート・比較列・マッチャー・差分モード等）はタブ①と同様。
...
**出力プレビュー**にシート名→出力ファイル名の対応が表示される（橙=正規表現不一致のフォールバック、赤=不正文字の `_` 置換）。
...
ファイル名が毎回変わる定期比較に使うペアリングパターンを管理する。3ステップのウィザード形式。

**ステップ①：フォルダ探索**
- 旧フォルダ・新フォルダを選択し、類似度しきい値（デフォルト 0.30）を設定して「探索」を実行する。
- 旧・新フォルダのパスと探索結果（自動ペア・未対応ファイル一覧）がログに表示される。

**ステップ②：正規表現パターン編集**
- 探索結果から正規表現を自動生成（「パターン生成 →」ボタン）。
- 自動生成は3段階のインテリジェント分析を実行：
  1. テンプレートスコアマッチング（スコア0.5以上で採用）
  2. `generate_regex()` によるパターン生成
  3. 最長共通前後缀（LCP/LCS）分析
- 正規表現は手動編集も可能。

**ステップ③：保存**
- パターンID・パターン名を入力して「JSON 保存」で `patterns.json` に追記する。
- 「そのまま比較」でタブ②へ切り替えてすぐに比較を実行することもできる。
