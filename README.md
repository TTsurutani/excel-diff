...
| F | 準比較キー（サブキー救済でマッチした行のみ `--sub-key-cols` の値が入る） |
| G | 項目（ヘッダー行から解決した列名。解決できない場合は列番号表記） |
| H / I | 旧項目値 / 新項目値（文字単位で変更部分をハイライトしたリッチテキスト） |

EQUAL行は出力しない（差分のみを列挙）。行全体の背景色は種類に応じて薄緑（追加）・薄赤（削除）・
薄黄（変更）で色分けされ、H・I列は変更部分が赤字取り消し線（削除側）・緑字太字（追加側）になる。
サブキー救済ペアの主キー列変更行のみ、H・I列が薄紫で上書きされる（行全体は塗らない）。
