...
例: シート名 `売上(Sales)` / `売上（Sales）` に `--name-regex "^([^(（]+)"` を適用 → `売上.xlsx`（半角・全角括弧の両方に対応）
...
| `--name-regex PATTERN` | シート名からファイル名ベースを抽出する正規表現（第1キャプチャグループを使用）。例: `"^([^(（]+)"` で括弧より前の名前を抽出（半角・全角両対応） | なし（シート名をそのまま使用） |
...
マッチャーの `matches()` / `normalize_old()` / `normalize_new()` に渡す値は、渡す前に必ず 5-4 のセル値正規化（None/空文字列の同一視、`_x000D_` 除去、改行コード統一）を経る。マッチャー自身が独自に改行コード等を正規化する必要はない（`equivalence` の `"-"`/`""` 同一視のように、正規化後の値に対する追加の同一視だけを担う）。
...
### 7-6. マッチャーの優先順位（列ごとの上書き）

1列に対して複数のマッチャーが該当する場合、`matchers` 配列の**先頭から順に走査し、最初に該当したマッチャーのみ**が使われる（以降のマッチャーは評価されない）。そのため `column: "*"` の全列ルールと特定列のルールを併用する場合は、**特定列のルールを `"*"` より前に**記述する必要がある。
...
上記の例では、G列・H列はそれぞれの `values` 集合が優先され、それ以外の列は `"*"` ルール（`"-"` と `""` の同一視）が適用される。順序を逆にすると `"*"` が先に該当してしまい、G列・H列の個別ルールが機能しない。

### 7-7. numeric タイプ（数値/数字文字列の型違いの同一視、issue #14）

セル値の比較は Python の `==` を使うため、表示上は同じ数字でも型（`int`/`float`/`str`）が異なると差分として検出される（例: CRM自動生成データの `128`（int）と手動メンテ原本の `'128'`（str、セル書式が文字列のため））。`numeric` タイプはこれを型を問わず数値として同一視する。
