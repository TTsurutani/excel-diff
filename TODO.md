...
### [TODO-004] 数値と数字文字列を同一視するマッチャー追加

**概要**
`128`（int）と`'128'`（str）のように、値としては同じでも型が異なるセルが差分として検出される。
数値変換できる値同士を型を問わず同一視する `numeric` マッチャーを追加する。

**詳細**: [Issue #14](https://github.com/TTsurutani/excel-diff/issues/14)
